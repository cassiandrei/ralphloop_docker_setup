...
if [ -z "$CLAUDE_CODE_OAUTH_TOKEN" ]; then
  echo "Warning: No OAuth token found. Headless mode may fail."
...
push_to_backup() {
  ...
  if git push origin HEAD 2>/dev/null; then
    ...
    echo "Push to remote failed (continuing anyway)"
  fi
}
...
EOF
  ...
  echo "" >> "$REPORT_FILE"
  grep '^\s*- \[x\]' "$PLAN_FILE" 2>/dev/null >> "$REPORT_FILE" || echo "None" >> "$REPORT_FILE"
